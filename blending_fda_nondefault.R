...
## Creating Flexible Disriminant Analysis model to be used in blended ensemble model. 
# Kylie Foster
...
## FDA -----
fda_grid <- expand.grid(.nprune = c(15, 20, 25, 30, 35), 
                         .degree = c(1, 2)) 
set.seed(seed)
tic()
fda_model <- train(damage_grade ~.,
                  data = train_layer1,
                  method = "fda", 
                  metric = metric,
                  tuneGrid = fda_grid,
                  trControl = my_control) # using default hyperparameters
toc() # 6432.92 sec elapsed

# Printing results
print(fda_model) # F1 = 0.6590109
# Plotting results
png("fda_nondefault.png")
plot(fda_model)
dev.off()

# save the model to disk
saveRDS(fda_model, "./fda_model_nondefault.rds")
